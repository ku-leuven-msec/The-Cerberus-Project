...
namespace interaction
{
	...
	// might run uncontrollably for a while before the signal handler associated
	...
	// Code from https://github.com/scanmem/scanmem/blob/master/ptrace.c
	/* Reads data from the target process, and places it on the `monitor_buffer`. */
	/* TODO fix - this method will fail to read data with size that is not multiple of 8 bytes */
	static size_t read_memory_ptrace(pid_t variantpid, void* variant_address, size_t data_len, void* monitor_buffer)
	{
		size_t nread;
		...
		errno = 0;
		for (nread = 0; nread < data_len; nread += sizeof(long)) {
			const char* ptrace_address = (char*)variant_address + nread;
			long ptraced_long = ptrace(PTRACE_PEEKDATA, variantpid, ptrace_address, NULL);
			...
			if (unlikely(ptraced_long == -1L && errno != 0)) {
				...
				if (errno == EIO || errno == EFAULT) {
					...
					for (j = 1, errno = 0; j < sizeof(long); j++, errno = 0) {
						...
						ptraced_long = ptrace(PTRACE_PEEKDATA, variantpid, ptrace_address - j, NULL);
						...
						memcpy((char*)monitor_buffer + nread, new_memory_ptr, sizeof(long) - j);
						nread += sizeof(long) - j;
			...
			memcpy((char*)monitor_buffer + nread, &ptraced_long, sizeof(long));
		}

		if (nread != data_len)
			warnf("interaction::read_memory_ptrace failed. tried to read %ld bytes - actually read %zd bytes - errno: %d (%s)\n",
				  data_len, nread, errno, strerror(errno));

		return nread;
	}

	/* Writes data to the target process memory. Works only for multiples of long data_len. */
	static size_t write_memory_multiple_of_long_len_ptrace(pid_t variantpid, void* variant_address, size_t data_len, void* monitor_buffer)
	{
		if (data_len % sizeof(long) != 0)
			warnf("Passing data len that is not multiple of sizeof(long)\n");

		size_t nwrite;
		unsigned long ptrace_data;

		/* Write the memory with `ptrace()`: the API specifies that `ptrace()` writes a `long`, which
		 * is the size of a word for the current architecture, so this section will deal in `long`s */
		errno = 0;
		for (nwrite = 0; nwrite < data_len; nwrite+=sizeof(long)) {
			memcpy(&ptrace_data, &(((char*)monitor_buffer)[nwrite]), sizeof(long));

			const char* ptrace_address = (char*)variant_address + nwrite;
			long ret = ptrace(PTRACE_POKEDATA, variantpid, ptrace_address, (void*)ptrace_data);

			/* check if ptrace() succeeded */
			if (unlikely(ret == -1L && errno != 0)) {
				warnf("write_memory_multiple_of_long_len_ptrace failed\n");
				return nwrite;
			}
		}
