...
public:
	...
	void add_pidstr(std::stringstream& ss)
	{
		if (mvee::active_monitor && variantnum > 0)
			ss << mvee::active_monitor->call_get_variant_pidstr() << " - ";
		else
...
class CerberusNotInitializedFailure: public MVEEBaseException
{
public:
	CerberusNotInitializedFailure(int _variantnum, const char* when)
	{
		variantnum = _variantnum;
		std::stringstream ss;
		...
		ss << " - Cerberus not initialized correctly - when: "
		   << when
		   << " - errno: "
		   << getTextualErrno(errno);
...
class PtraceOpFailure: public MVEEBaseException
{
public:
	PtraceOpFailure(int _variantnum, const char* when)
	{
		variantnum = _variantnum;
		std::stringstream ss;
		...
		ss << " - Ptrace operation failed or non expected result - when: "
		   << when
		   << " - errno: "
		   << getTextualErrno(errno);
