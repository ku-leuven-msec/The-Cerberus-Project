...
/*******************************************************************************
 * TODO check if all of these are still needed for Cerberus
 * Cerberus can deliver signal in one of three ways:
 ...
 *
 * 2) if the signal is asynchronous, Cerberus will always deny the delivery of
 ...
 * argument here. Cerberus does however cache _all_ of the signal's information.
 * It then waits for a rendez-vous point.
 *
 * 2.1) if the first rendez-vous point is the return site of an interrupted
 * sys_[rt_]sigsuspend call, Cerberus will first restart the call and at the NEXT
 * rendez-vous point (i.e. the entry site of the restarted call), Cerberus
 ...
 * so Cerberus will inject the signal using:
 * ptrace(PTRACE_SYSCALL, pid, NULL, sig);
 *
 * We do of course have to adjust the sender PID of the signal before the final
 * injection.
 *
 ...
 * the syscall no by __NR_getpid an then transfer the control to the infinite
 * loop function in Cerberus's glibc.
 *
...
{
	for (int i = 0; i < _NSIG-1; ++i) {
		if (action_table[i].sa_handler && action_table[i].sa_handler != SIG_IGN)
