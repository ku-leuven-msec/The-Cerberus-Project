...
#define WEAK_INIT_KEY(key, value) if (!key) key = value;
	...
	WEAK_INIT_KEY(config["variant"]["global"]["settings"]["use_erim_binary_rewriting_libs"], false);
	WEAK_INIT_KEY(config["monitor"]["erim_binary_rewriting_libs_path"]["path"], "/erim_binary_rewriting_libs/");

	if ((*mvee::config_variant_global)["use_erim_binary_rewriting_libs"].asBool()) {
		bool is_relative = !(*mvee::config_monitor)["erim_binary_rewriting_libs_path"]["is_absolute"].asBool();
		mvee::add_library_path((*mvee::config_monitor)["erim_binary_rewriting_libs_path"]["path"].asCString(),
							   is_relative, is_relative);
	}

	WEAK_INIT_KEY(config["variant"]["global"]["settings"]["use_custom_openssl"], false);
	WEAK_INIT_KEY(config["monitor"]["custom_openssl_path"]["path"], "/patched_binaries/openssl/erimized/");

	if ((*mvee::config_variant_global)["use_custom_openssl"].asBool()) {
		bool is_relative = !(*mvee::config_monitor)["custom_openssl_path"]["is_absolute"].asBool();
		mvee::add_library_path((*mvee::config_monitor)["custom_openssl_path"]["path"].asCString(),
							   is_relative, is_relative);
	}
}
...
{
	const char* parsec_bench  = nullptr;
	const char* parsec_config = nullptr;
	const char* splash_bench  = nullptr;
	const char* spec_bench    = nullptr;
	char parsec_ver = 2;
	bool native = false;
